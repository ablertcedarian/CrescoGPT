...
def runner():
    ...
    init_from = 'resume'
    init_file_name = f'baseline-{arch_name}-ckpt-3375.pt'
    ckpt_path = f'/openwebtext/{init_file_name}'
    # resume_run_id = 'ttyi4ny5'
    ...
    expansion_k = 3 # ideally this would scale inversely with flops 
    ...
    def decide_expansion(model, history):
        if len(history['val_loss']) < expansion_k:
            return False

        rate_of_improvement = (history['val_loss'][-expansion_k] - history['val_loss'][-1]) / (np.log(sum(history['flops'])) - np.log(sum(history['flops'][:-expansion_k + 1])))
        r_threshold = gamma * history['val_loss'][-1]
        return rate_of_improvement < r_threshold
    ...
    t0 = time.time()
    t1 = None
    history = dict(flops=[], val_loss=[])
    current_arch = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size)
        ...
        if iter_num % eval_interval == 0 and master_process:
            ...
            if (losses['val'] < best_val_loss or always_save_checkpoint):
                ...
                if iter_num > 0:
                    ...
                    model_name = f"baseline-{arch_name}"
